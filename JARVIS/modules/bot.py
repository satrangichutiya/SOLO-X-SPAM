...
async def manage_sudo_users(event, add):
    ...
    ok = await event.reply(f"» __Jarvis Ka Ek Beta Aur Add Ho rha hai..__" if add else "YE Jarvis Ki Najayaz Aulad thi isiliye nikal diya💋...")
    target = ""
    ...
    try:
        ...
        await ok.edit("» BSDK...REPLY KRKE KAR !!" if add else "Reply to a message to remove the user.")
        return
    if add:
